...
public sealed record MonsterState
{
    ...
    public ImmutableDictionary<string, int> WoundsByLocation { get; init; } =
...
public sealed record SurvivorCombatState(
    string SurvivorId, string Name, SurvivorAttributes Attributes, string WeaponId, bool Dead);

public sealed record DrawnLocation(
    string CardId, bool HasCriticalSlot, bool IsTrap, int WoundsOn, WoundOutcome? Result);

public sealed record AttackSequence(
    string AttackId, string SurvivorId, string WeaponId, string TargetMonsterId,
    int AttackDice, int HitsOn, int? EnteredHitCount, ImmutableArray<DrawnLocation> DrawnLocations);
...
{
    public static RngCursors Seeded(ulong masterSeed) =>
        new(masterSeed, ImmutableDictionary<string, ulong>.Empty);
}
