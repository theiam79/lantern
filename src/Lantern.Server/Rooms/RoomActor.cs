...
)
{
    ...
    public async Task<CreateRoomResult> InitNewAsync(CreateRoomRequest req, string connectionId)
    {
        ...
        try
        {
            ...
            db.Rooms.Add(
                ...
                {
                    RoomCode = roomCode,
                    ContentPackId = req.ContentPackId,
                    RoomPasswordHash = _roomPasswordHash,
            ...
            db.RoomMembers.Add(
                new RoomMemberRow
                {
                    ...
                    DisplayName = req.DisplayName,
                    IsHost = true,
                    PlayerTokenHash = _roster[playerId].TokenHash,
                    JoinedAt = now,
                    LastSeenAt = now,
                }
            );
    ...
    public async Task RehydrateAsync()
    {
        ...
        foreach (var m in await db.RoomMembers.Where(m => m.RoomCode == roomCode).ToListAsync())
            _roster[m.PlayerId] = new Member
            {
                PlayerId = m.PlayerId,
                DisplayName = m.DisplayName,
                IsHost = m.IsHost,
                TokenHash = m.PlayerTokenHash,
            };

        var snap = await db
            .Snapshots.Where(s => s.RoomCode == roomCode)
        ...
        logger.LogInformation(
            "Rehydrated room {Room} at seq {Seq} ({Members} members)",
            roomCode,
            _lastSeq,
            _roster.Count
        );
    ...
    public async Task<JoinRoomResult> JoinAsync(JoinRoomRequest req, string connectionId)
    {
        ...
        {
            if (
                _roomPasswordHash is not null
                && (req.RoomPassword is null || Crypto.Hash(req.RoomPassword) != _roomPasswordHash)
            )
    ...
    public async Task<JoinRoomResult> ResumeAsync(ResumeRequest req, string connectionId)
    {
        ...
        {
            if (
                !_roster.TryGetValue(req.PlayerId, out var m)
                || !Crypto.FixedTimeEquals(Crypto.Hash(req.PlayerToken), m.TokenHash)
            )
    ...
    public async Task BroadcastPresenceAsync()
    {
        ...
        try
        {
            await hub
                ...
                .Presence(new ServerMessage<RosterDto>(Protocol.Version, _lastSeq, "presence", Roster()));
        }
        finally
        {
            _gate.Release();
    ...
    public async Task MarkDisconnectedAsync(string playerId, string connectionId)
    {
        ...
        try
        {
            if (_roster.TryGetValue(playerId, out var m) && m.ConnectionId == connectionId)
            {
                m.Connected = false;
                m.ConnectionId = null;
            }
    ...
    public async Task<IntentAck> SubmitAsync(IntentEnvelope env)
    {
        ...
        {
            if (
                !_roster.TryGetValue(env.PlayerId, out var member)
                || !Crypto.FixedTimeEquals(Crypto.Hash(env.PlayerToken), member.TokenHash)
            )
            ...
            {
                intent =
                    JsonSerializer.Deserialize<Intent>(env.Intent.GetRawText(), Json)
                    ?? throw new JsonException("null");
            }
            catch (Exception)
            {
                return new IntentAck(false, null, "intent_rejected", env.ClientIntentId);
            }
    ...
    private async Task<IntentAck> UndoAsync(UndoIntent undo, string actorPlayerId, string clientIntentId)
    {
        ...
        using (var scope = scopes.CreateScope())
        {
            ...
            // edited content pack.
            var baseSnap = await db
                .Snapshots.Where(s => s.RoomCode == roomCode && s.Seq < minRev)
                .OrderByDescending(s => s.Seq)
                .FirstOrDefaultAsync();
            var baseState = baseSnap?.StateJson is { } sj ? JsonSerializer.Deserialize<ShowdownState>(sj, Json) : null;
            var baseSeq = baseSnap?.Seq ?? -1;
            var rows = await db
                .Journal.Where(j => j.RoomCode == roomCode && j.Seq > baseSeq)
                .OrderBy(j => j.Seq)
                .ToListAsync();
            ...
            db.Journal.Add(
                new JournalRow
                {
                    ...
                    RevertedSeqsJson = JsonSerializer.Serialize(toRevert),
                    ClientIntentId = clientIntentId,
                    CommittedAt = now,
                }
            );
    ...
    private async Task<IntentAck> SetRoomPasswordAsync(SetRoomPasswordIntent sp, string clientIntentId)
    {
        ...
        if (room is not null)
        {
            room.RoomPasswordHash = _roomPasswordHash;
    ...
    private SnapshotRow SnapshotRowFor(long seq, ShowdownState? state, DateTimeOffset at) =>
        new()
        {
            RoomCode = roomCode,
            Seq = seq,
            ContractVersion = Protocol.Version,
            StateJson = state is null ? null : JsonSerializer.Serialize(state, Json),
            At = at,
        };
    ...
    private async Task UpdateRoomAsync(LanternDbContext db, long seq, ShowdownState? state, DateTimeOffset now)
    {
        ...
        room.UpdatedAt = now;
        room.Status =
            state?.Status == ShowdownStatus.Ended ? "ended"
            : state is not null ? "showdown"
            : room.Status;
    }
    ...
    private async Task BestEffortPruneAsync()
    {
        try
        {
            ...
            if (stale.Count > 0)
            {
                db.Snapshots.RemoveRange(stale);
                await db.SaveChangesAsync();
            }
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "snapshot prune failed for {Room} (non-fatal)", roomCode);
        }
    }

    private void IndexCommitted(JournalEntry e)
    {
        if (e.ClientIntentId is { } cid)
            _committedIntents[cid] = e.Seq;
        if (e.Seq == 0)
            return;
        foreach (var r in e.RevertedSeqs)
            _reverted.Add(r);
        if (e.Intent is not null and not UndoIntent)
            _gameplaySeqs.Add(e.Seq);
    }

    private static JournalEntry ToEntry(JournalRow r) =>
        new(
            r.RoomCode,
            r.Seq,
            r.ActorPlayerId,
