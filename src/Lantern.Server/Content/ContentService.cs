...
file sealed record PackFile(
    int SchemaVersion,
    string PackId,
    WeaponJson[]? Weapons,
    MonsterJson[]? Monsters,
    FormulaJson? WoundFormula
);
...
public sealed class ContentService
{
    ...
    public ContentService(IHostEnvironment env, IConfiguration config, ILogger<ContentService> log)
    {
        ...
        _log.LogInformation(
            "Loaded {PackCount} content pack(s), {DeckCount} host deck(s) from {Root}.",
            _packs.Count,
            hostDecks.Count,
            root
        );
    ...
    private Dictionary<string, IReadOnlyList<HitLocCardDef>> LoadHostDecks(string dir)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(dir, "*.json"))
        {
            try
            {
                ...
                if (d?.DeckId is null || d.Cards is null)
                    continue;
                var cards = d
                    .Cards.Select(c => new HitLocCardDef(
                        c.Id,
                        c.Label,
                        c.HasCriticalSlot,
                        c.IsTrap,
            ...
            {
                _log.LogError(ex, "Failed to load host deck {File}", file);
            }
        }
        return decks;
    }
        ...
        foreach (var file in Directory.EnumerateFiles(dir, "*.json"))
        {
            try
            {
                ...
                var gear = (p.Weapons ?? []).ToDictionary(
                    w => w.Id,
                    w => new GearDef(w.Id, w.Name, w.Speed, w.Accuracy, w.Strength)
                );
                ...
                var formula = new FormulaConfig(
                    WoundMin: p.WoundFormula?.Min ?? 2,
                    WoundMax: p.WoundFormula?.Max ?? 10
                );
                _packs[p.PackId] = new ContentPack(p.PackId, formula, monsters, gear, hostDecks);
            }
            catch (Exception ex)
            {
                _log.LogError(ex, "Failed to load content pack {File}", file);
            }
        }
    }
