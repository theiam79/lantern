...
public static class ShowdownEngine
{
    ...
    public static ShowdownState? Fold(ShowdownState? snapshot, IReadOnlyList<JournalEntry> tail, IContentPack pack)
    {
        ...
        {
            if (e.Seq == 0 || e.Intent is null or UndoIntent)
                continue; // genesis / no-op revert markers
            if (reverted.Contains(e.Seq))
                continue;
            var r = Reduce(state, e.Intent, e.Seq, pack);
            if (r.Accepted)
                state = r.State; // a replayed reject is a bug; keep prior state
        }
        return state;
    }
    ...
    private static ReduceResult StartShowdown(ShowdownState? state, StartShowdownIntent si, long seq, IContentPack pack)
    {
        if (state is not null)
            return ReduceResult.Reject("already-started");
        if (si.MasterSeed is null)
            return ReduceResult.Reject("seed-not-resolved");
        if (!pack.TryGetLevel(si.MonsterId, si.MonsterLevelId, out var level))
            return ReduceResult.Reject("unknown-monster-level");

        var cards = pack.GetHitLocationDeck(level.HitLocationDeckRef);
        if (cards is null || cards.Count == 0)
            return ReduceResult.Reject("missing-hit-location-deck");
    ...
    {
        if (state.Status == ShowdownStatus.Ended)
            return ReduceResult.Reject("showdown-ended");
        var attrs = new SurvivorAttributes(
            a.Attributes.Mov,
            a.Attributes.Acc,
            a.Attributes.Str,
            a.Attributes.Eva,
    ...
    {
        if (state.Status != ShowdownStatus.Idle)
            return ReduceResult.Reject("not-idle");
        if (!state.Survivors.TryGetValue(d.SurvivorId, out var surv))
            return ReduceResult.Reject("unknown-survivor");
        if (!pack.TryGetGear(d.WeaponId, out var weapon))
            return ReduceResult.Reject("unknown-weapon");
    ...
    {
        if (state.Status != ShowdownStatus.AwaitingHits || state.Attack?.AttackId != e.AttackId)
            return ReduceResult.Reject("bad-step");
        var count =
            e.Mode == "rolls" ? (e.Rolls?.Count(r => r >= state.Attack.HitsOn) ?? 0) : Math.Max(0, e.HitCount ?? 0);
        var status = count <= 0 ? ShowdownStatus.Resolved : ShowdownStatus.DrawingLocations;
        return ReduceResult.Ok(
            state with
            {
                Attack = state.Attack with { EnteredHitCount = count },
                Status = status,
                LastSeq = seq,
            }
        );
    ...
    private static ReduceResult DrawHitLocations(
        ShowdownState state,
        DrawHitLocationsIntent dl,
        long seq,
        IContentPack pack
    )
    {
        if (state.Status != ShowdownStatus.DrawingLocations || state.Attack?.AttackId != dl.AttackId)
            return ReduceResult.Reject("bad-step");
        if (!pack.TryGetLevel(state.MonsterId, state.MonsterLevelId, out var level))
            return ReduceResult.Reject("unknown-monster-level");
        var cards = pack.GetHitLocationDeck(level.HitLocationDeckRef);
        if (cards is null)
            return ReduceResult.Reject("missing-hit-location-deck");
        if (!pack.TryGetGear(state.Attack.WeaponId, out var weapon))
            return ReduceResult.Reject("unknown-weapon");
        if (!state.Survivors.TryGetValue(state.Attack.SurvivorId, out var surv))
            return ReduceResult.Reject("unknown-survivor");
        ...
        var (drawnIds, deck, cursors) = HitLocationDeck.Draw(
            ...
            count,
            state.Rng,
            id => byId.TryGetValue(id, out var d) && d.IsTrap
        );
    ...
    {
        if (state.Status != ShowdownStatus.AwaitingWounds || state.Attack?.AttackId != w.AttackId)
            return ReduceResult.Reject("bad-step");
        var locs = state.Attack.DrawnLocations;
        var idx = -1;
        for (var i = 0; i < locs.Length; i++)
            if (locs[i].CardId == w.LocationCardId && locs[i].Result is null)
            {
                idx = i;
                break;
            }
        if (idx < 0)
            return ReduceResult.Reject("location-not-found-or-resolved");

        var loc = locs[idx];
        var outcome =
            w.Mode == "roll" ? ComputeWound(w.Roll, loc.WoundsOn, loc.HasCriticalSlot) : ParseOutcome(w.Outcome);
    ...
    {
        if (state.Status != ShowdownStatus.Resolved || state.Attack?.AttackId != ap.AttackId)
            return ReduceResult.Reject("bad-step");
        ...
        try
        {
            ...
            if (path == "monster.totalWounds")
                return ReduceResult.Ok(
                    state with
                    {
                        Monster = state.Monster with { TotalWounds = patch.Value.GetInt32() },
                        LastSeq = seq,
                    }
                );
            if (path.StartsWith("monster.woundsByLocation.", StringComparison.Ordinal))
            {
                ...
                return ReduceResult.Ok(
                    state with
                    {
                        Monster = state.Monster with { WoundsByLocation = w },
                        LastSeq = seq,
                    }
                );
            }
            if (
                path.StartsWith("survivor.", StringComparison.Ordinal)
                && path.EndsWith(".dead", StringComparison.Ordinal)
            )
            {
                var id = path["survivor.".Length..^".dead".Length];
                if (!state.Survivors.TryGetValue(id, out var s))
                    return ReduceResult.Reject("unknown-survivor");
            ...
            if (
                path.StartsWith("attack.", StringComparison.Ordinal)
                && path.EndsWith(".outcome", StringComparison.Ordinal)
            )
            {
                if (state.Attack is null)
                    return ReduceResult.Reject("no-attack-in-flight");
                var cardId = path["attack.".Length..^".outcome".Length];
                var locs = state.Attack.DrawnLocations;
                var idx = -1;
                for (var i = 0; i < locs.Length; i++)
                    if (locs[i].CardId == cardId)
                    {
                        idx = i;
                        break;
                    }
                if (idx < 0)
                    return ReduceResult.Reject("location-not-found");
    ...
    {
        if (roll is null)
            return WoundOutcome.Fail;
        if (roll == 10 && hasCrit)
            return WoundOutcome.Crit;
        return roll >= woundsOn ? WoundOutcome.Wound : WoundOutcome.Fail;
    }

    private static WoundOutcome ParseOutcome(string? outcome) =>
        outcome switch
        {
